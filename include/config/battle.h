...
// Type settings
#define B_GHOSTS_ESCAPE             GEN_5 // In Gen6+, abilities like Shadow Tag or moves like Mean Look fail on Ghost-type Pokémon. They can also escape any Wild Battle.
...
#define B_MEMENTO_FAIL              GEN_3 // In Gen4+, Memento fails if there is no target or if the target is protected or behind substitute. But not if Atk/Sp. Atk are at -6.
...
#define B_ALLY_SWITCH_FAIL_CHANCE   GEN_8 // In Gen9, using Ally Switch consecutively decreases the chance of success for each consecutive use.
...
#define B_INTREPID_SWORD            GEN_L8 // In Gen9+, Intrepid Sword raises Attack by one stage only once per Battle.
...
#define B_STEAL_WILD_ITEMS          GEN_8 // In Gen9, Thief and Covet steal a wild pokemon's item and send it to the bag. Before Gen9, the stolen item would be held by the Thief/Covet user.
...
#define B_NET_BALL_MODIFIER         GEN_LATEST // In Gen7+, Net Ball's catch multiplier is x3.5 instead of x3.
...
#define B_NUM_LOW_HEALTH_BEEPS          2 // This controls the number of times the "low health" beep will loop. Setting this value to NUM_BEEPS_OFF will disable the beep, while NUM_BEEPS_GEN_3 will loop infinitely. You can set this to any number you want, the defines listed are just for ease of use.
